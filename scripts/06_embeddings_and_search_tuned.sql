...
""", project_id, dataset, embed_model, project_id, dataset);

-- ==============================================
-- Case embeddings (include parsed theme in content)
-- ==============================================
EXECUTE IMMEDIATE FORMAT("""
  ...
  FROM ML.GENERATE_EMBEDDING(
    ...
    (
      SELECT
        service_request_id, theme, summary,
...
-- ===============================================
-- Vector search pool (top_k per service_request_id)
-- ===============================================
EXECUTE IMMEDIATE FORMAT("""
...
-- ===============================================
-- Stage 1: Precise (theme-consistent) best match
-- ===============================================
EXECUTE IMMEDIATE FORMAT("""
  ...
  SELECT * EXCEPT(rn) FROM (
    SELECT
      m.*,
      ROW_NUMBER() OVER (PARTITION BY m.service_request_id ORDER BY m.cosine_distance ASC) AS rn
    FROM `%s.%s._matches_all_vsearch` m
    WHERE LOWER(m.case_theme) = LOWER(m.target_theme)
  )
  WHERE rn = 1 AND cosine_distance <= %f
""", project_id, dataset, project_id, dataset, cutoff);

-- ===============================================
-- Stage 2: Global fallback (only if no precise)
-- ===============================================
EXECUTE IMMEDIATE FORMAT("""
