...
-- 1) Stage: all summarized cases + any match
EXECUTE IMMEDIATE FORMAT("""
  CREATE OR REPLACE TABLE `%s.%s._proto_pool` AS
  SELECT
    s.service_request_id,
    s.summary,
    s.summary_source,                         -- 'text' or 'image'
    SAFE.PARSE_JSON(r.out_text) AS tri_obj,  -- existing triage JSON
    m.policy_title,
    m.target_theme,
    m.source_url,
    m.cosine_distance,
...
-- 2) Parse theme/severity/action safely
EXECUTE IMMEDIATE FORMAT("""
...
-- 3) Build prototype_1000:
--    prefer policy-matched, then closest cosine_distance, then id; keep only summarized rows
EXECUTE IMMEDIATE FORMAT("""
  ...
  LIMIT %d
""", project_id, dataset, project_id, dataset, proto_limit);

-- 4) Balanced 400 slice = 200 text + 200 image (priority: matched → nearest)
EXECUTE IMMEDIATE FORMAT("""
...
""", project_id, dataset, project_id, dataset, per_source_n, per_source_n);

-- 5) Chart-ready cohort metrics (No-AI vs With-AI)
EXECUTE IMMEDIATE FORMAT("""
